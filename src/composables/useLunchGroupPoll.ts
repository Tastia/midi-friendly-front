...
export function useLunchGroupPoll(poll: MapLunchGroupPoll) {
  ...
  const users = computed<GatewayUser[]>(
    () =>
      poll.votes
        .map((vote) => mapGatewayApi?.users.value.find((user) => user._id === vote.user) ?? null)
  ...
  const voteState = computed(() => {
    const restaurantIds = [
      ...new Set([...poll.votes.map((vote) => vote.restaurant), ...(poll?.allowedRestaurants ?? [])]),
    ];
    ...
    return restaurantIds
      .map((restaurantId) => ({
        ...
        voteCount: {
          total: poll.votes.filter((vote) => vote.restaurant === restaurantId).length,
          percentage: Math.round(
            (poll.votes.filter((vote) => vote.restaurant === restaurantId).length / poll.votes.length) * 100
          ),
        },

        users: poll.votes
          .filter((vote) => vote.restaurant === restaurantId)
          .map((vote) => mapGatewayApi?.users.value.find((user) => user._id === vote.user) as GatewayUser),
      })) // SORT BY NAME
      .sort((a, b) => {
  ...
  const timeLeftPercentage = useTimerPercentage(poll.voteDeadline, poll.createdAt);

  const userVoteValue = computed<string | null>(
    () => poll.votes.find((vote) => vote.user === userStore.user?._id)?.restaurant ?? null
  );
  ...
  const voteOptions = computed(() => {
    if (poll.allowedRestaurants)
      return poll.allowedRestaurants.map((restaurantId) => ({
        label: mapGatewayApi?.restaurants.value.find((restaurant) => restaurant._id === restaurantId)
          ?.name as string,
        value: restaurantId,
      }));
