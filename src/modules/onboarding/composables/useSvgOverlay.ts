...
export default function useSvgOverlay() {
  ...
  const paddingRef = ref<SvgOverlayOptions["padding"]>(
    defaultVOnboardingWrapperOptions.overlay?.padding ?? 0
  );
  const borderRadiusRef = ref<SvgOverlayOptions["borderRadius"]>(
    defaultVOnboardingWrapperOptions.overlay?.borderRadius ?? 0
  );
