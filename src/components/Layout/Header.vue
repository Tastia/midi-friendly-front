  ...
  >
    ...
    <div class="flex items-center gap-2">
      <template v-if="showLogin && !userStore.user">
        <NButton type="primary" size="small" @click="$router.push('/auth/login')"> Connexion </NButton>
        <NDivider vertical class="!m-0" />
      </template>
      <template v-if="showLogin && userStore.user">
        <NButton type="primary" size="small" @click="$router.push('/map')"> Ouvrir l'app </NButton>
        <NDivider vertical class="!m-0" />
      </template>
